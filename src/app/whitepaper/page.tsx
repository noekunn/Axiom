...
const LogoIcon = () => (
  <svg className="w-9 h-9 text-[#ffffff]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default function WhitepaperPage() {
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-[#e7e4ee] font-sans selection:bg-[#ffffff]/30 selection:text-[#ffffff] overflow-x-hidden relative">
      ...
      {/* Grid Background & Ambient Shadows */}
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        ...
        />
        <div className="absolute top-[-5%] left-[-10%] w-[60%] h-[50%] rounded-full bg-[#ffffff]/5 blur-[130px]" />
        <div className="absolute bottom-[10%] right-[5%] w-[55%] h-[55%] rounded-full bg-[#A8A8A8]/4 blur-[140px]" />
      </div>

      {/* HEADER */}
      <header className="sticky top-0 z-50 w-full bg-[#080808]/80 backdrop-blur-xl border-b border-[#1e1e1e]">
        <div className="max-w-[1200px] mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-zinc-500 text-xs px-2.5 py-1 rounded bg-[#1e1e1e] font-mono">v2.4-PROD</span>
          </div>

          <div className="flex items-center gap-4">
            <Link 
              href="/" 
              className="text-xs font-semibold text-zinc-400 hover:text-white border border-[#1e1e1e] hover:bg-white/5 px-4 py-2 rounded transition-all duration-200 flex items-center gap-1.5"
            >
              <ArrowLeft className="w-3.5 h-3.5" /> Back to Home
            </Link>
            <Link 
              href="/docs" 
              className="text-xs font-semibold text-zinc-400 hover:text-white border border-[#1e1e1e] hover:bg-white/5 px-4 py-2 rounded transition-all duration-200"
            >
              Docs
            </Link>
            <Link 
      ...
      <main className="max-w-[1200px] mx-auto px-6 py-12 relative z-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <aside className="lg:col-span-1">
          <div className="sticky top-24 p-5 rounded-lg border border-[#1e1e1e] bg-[#0e0e0e]">
            <h3 className="font-display font-bold text-xs text-zinc-500 mb-4 uppercase tracking-widest">
              Whitepaper Sections
            </h3>
            <nav className="flex flex-col gap-1">
              {sections.map((sec) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 rounded text-xs font-semibold tracking-tight transition-all duration-200 flex items-center justify-between ${
                    activeSection === sec.id
                      ? "bg-white/8 text-white border border-white/15 font-bold"
                      : "text-zinc-500 hover:text-white hover:bg-white/5 border border-transparent"
                  }`}
                >
                  {sec.label}
                  <ChevronRight className={`w-3 h-3 transition-opacity ${activeSection === sec.id ? 'opacity-100' : 'opacity-40'}`} />
                </button>
              ))}
            </nav>
            <div className="mt-6 pt-5 border-t border-white/5 flex flex-col gap-3">
              <a 
                href="/axiom-whitepaper.pdf"
                target="_blank"
                rel="noopener noreferrer"
                className="w-full text-center py-2.5 rounded bg-white hover:bg-zinc-200 text-black text-xs font-bold font-display shadow-md transition-all flex items-center justify-center gap-2"
              >
        ...
        <section className="lg:col-span-3">
          <div className="p-8 lg:p-12 rounded-lg border border-[#1e1e1e] bg-[#0e0e0e] shadow-2xl space-y-12">
            
            {/* Section 1: Abstract */}
            {activeSection === "abstract" && (
              <div className="space-y-6 animate-fade-in">
                <div className="flex items-center gap-3 text-[#A8A8A8] font-mono text-xs uppercase tracking-widest">
                ...
                <div className="p-5 rounded border border-[#1e1e1e] bg-white/[0.03] border-l-4 border-l-[#A8A8A8] my-6">
                  <h4 className="text-xs font-bold font-display text-[#C0C0C0] uppercase tracking-widest mb-2">Architectural Hypothesis</h4>
            ...
            {activeSection === "royalty-model" && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="my-8 overflow-hidden rounded border border-[#1e1e1e] bg-white/[0.03] p-6 space-y-4">
                  <h3 className="text-sm font-bold font-display text-white">Compounding Yield Mechanics</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 rounded bg-[#080808] border border-[#1e1e1e]">
                      <h4 className="text-[11px] font-mono text-[#C0C0C0] uppercase tracking-wider mb-2">Upfront Payout Vector</h4>
                      <p className="text-xs text-zinc-400">Experts claim and resolve tasks inside specific domain vetting pools, instantly earning baseline upfront points mapped directly into sovereign payouts (Razorpay/Stripe).</p>
                    </div>
                    <div className="p-4 rounded bg-[#080808] border border-[#1e1e1e]">
                      <h4 className="text-[11px] font-mono text-[#A8A8A8] uppercase tracking-wider mb-2">Passive Royalty Stream</h4>
                      <p className="text-xs text-zinc-400">Whenever an enterprise purchaser licenses that specific asset pool to reinforce their LLMs, secondary marketplace royalties (up to 20%) are routed directly back to the original validators.</p>
                    </div>
                  </div>
                </div>
                <p className="text-sm text-zinc-400 leading-relaxed font-sans">
            ...
            {activeSection === "consensus" && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="p-6 rounded-lg border border-[#1e1e1e] bg-white/[0.03] flex flex-col md:flex-row gap-6 items-center">
                  <div className="w-12 h-12 rounded bg-white/5 flex items-center justify-center text-[#C0C0C0] shrink-0">
            ...
            {activeSection === "vetting-math" && (
              <div className="space-y-6 animate-fade-in">
                ...
                <hr className="border-white/5" />
                <p className="text-sm text-zinc-400 leading-relaxed font-sans">
                  Axiom contributors are not anonymous crowdfinders; they are vetted domain expert nodes. To qualify for specific asset pools, contributors must enter the Vetting Arena, passing automated time-based challenges mapped directly to their domains.
                </p>
                <p className="text-sm text-zinc-400 leading-relaxed font-sans">
                  Based on their performance in the Vetting Arena, nodes receive structural tiers that multiply their relocation points:
                </p>
                <div className="overflow-x-auto rounded border border-[#1e1e1e] bg-white/[0.02]">
                  <table className="w-full text-xs text-left">
                    <thead className="bg-[#080808] border-b border-[#1e1e1e] text-zinc-400">
                      <tr>
                        <th className="px-4 py-3 font-mono uppercase tracking-wider text-[10px]">Node Tier Level</th>
                        <th className="px-4 py-3 font-mono uppercase tracking-wider text-[10px]">Minimum XP Needed</th>
                        <th className="px-4 py-3 font-mono uppercase tracking-wider text-[10px]">Base Point Multiplier</th>
                        <th className="px-4 py-3 font-mono uppercase tracking-wider text-[10px]">Access Priority</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#1e1e1e] text-zinc-400">
                      ...
                      <tr className="hover:bg-white/[0.02] transition-colors">
                        <td className="px-4 py-3 font-semibold text-[#C0C0C0]">Elite Node</td>
                        <td className="px-4 py-3 font-mono text-[#C0C0C0]">10,000 XP</td>
                        <td className="px-4 py-3 font-mono text-[#C0C0C0]">2.50x</td>
                        <td className="px-4 py-3 text-[#C0C0C0]">Instant Automated Claims</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            {activeSection === "governance" && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded border border-[#1e1e1e] bg-[#080808]">
                    <h4 className="text-xs font-bold text-white font-display mb-2">Differential Privacy (DP) Filters</h4>
                    <p className="text-xs text-zinc-400">Axiom injects advanced differential noise checks into pre-training prompt corpuses, preventing generative models from memorizing personal identifying info (PII).</p>
                  </div>
                  <div className="p-4 rounded border border-[#1e1e1e] bg-[#080808]">
                    <h4 className="text-xs font-bold text-white font-display mb-2">ZKP Specialist Verification</h4>
                    <p className="text-xs text-zinc-400">Zero-knowledge proof verification algorithms check the credentials and academic degrees of professional contributor nodes, protecting their identities while proving expertise parameters.</p>
                  </div>
                </div>
                <p className="text-sm text-zinc-400 leading-relaxed font-sans">
      ...
      {/* FOOTER */}
      <footer className="border-t border-[#1e1e1e] bg-[#080808] z-20 relative py-12 text-xs text-zinc-500 font-mono mt-12">
